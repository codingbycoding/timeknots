var TimeKnots = {
  draw: function(id, events, options) {
    ...
    var format = cfg.dateDimension
      ? d3.time.format(cfg.labelFormat)
      : function(d) {
          return d;
        };
      ...
      events.map(function(d) {
        ...
        svg
          ...
          .attr('y', function(d) {
            if (cfg.horizontalLayout) {
              return Math.floor(
                cfg.height / 2 + (margin + this.getBBox().height),
              );
            }
	  ...
	  */
      ...
      svg
        ...
        .attr('x', function(d) {
          if (cfg.horizontalLayout) {
            return (
              cfg.width -
              d3.max([this.getBBox().width, margin + this.getBBox().width / 2])
            );
          }
          return Math.floor(this.getBBox().width / 2);
        })
      ...
      svg
        .on('mousemove', function() {
          ...
          return tip
            .style('top', d3.event.pageY - tipPixels - margin + 'px')
